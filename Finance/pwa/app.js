...
const DB = {
    ...
    set(key, value) { const db = this.load();
        db[key] = value;
        this.save(db); },
    ...
    addTxn(txn) { const t = this.getTxns();
        t.unshift({...txn, id: 'tx' + Date.now(), date: new Date().toISOString() });
...
const Nav = {
    ...
    back() { this.stack.pop();
        this.go(this.stack[this.stack.length - 1] || 'dashboard'); },
};
...
const Payments = {
    ...
    send() {
        ...
        Modal.confirm('Send ' + amt.toLocaleString() + ' RWF to ' + (toUser ? toUser.name : recip) + '?\nFee: ' + fee.toLocaleString() + ' RWF\nTotal deducted: ' + total.toLocaleString() + ' RWF', () => {
            user.balance -= total;
            user.creditScore = Math.min(850, (user.creditScore || 500) + 3);
            DB.saveUser(user);
            if (toUser) { toUser.balance += amt;
                toUser.creditScore = Math.min(850, (toUser.creditScore || 500) + 2);
                DB.saveUser(toUser);
                DB.addTxn({ userId: toUser.id, toUserId: user.id, type: 'receive', amount: amt, description: 'Received from ' + user.name + (note ? ' – ' + note : '') }); }
    ...
    topup() {
        ...
        Modal.confirm('Top up ' + amt.toLocaleString() + ' RWF to your wallet?', () => {
            const user = Auth.currentUser();
            user.balance += amt;
            user.creditScore = Math.min(850, (user.creditScore || 500) + 1);
            DB.saveUser(user);
...
const Loans = {
    renderPage() {
        ...
        if (user.activeLoan) {
            const loan = user.activeLoan,
                due = new Date(loan.dueDate).toLocaleDateString('en-RW', { day: 'numeric', month: 'short', year: 'numeric' });
    ...
    _preview() {
        ...
        const cfg = Config.get(),
            p = document.getElementById('loan-fee-preview');
    ...
    apply() {
        const user = Auth.currentUser(),
            amt = parseFloat(document.getElementById('loan-amount').value),
            days = parseInt(document.getElementById('loan-term').value),
            cfg = Config.get();
        if (!amt || amt < 500) { UI.toast('Min 500 RWF.', 'error'); return; }
        ...
        Modal.confirm('Borrow ' + amt.toLocaleString() + ' RWF?\nInterest: ' + interest.toLocaleString() + ' RWF\nRepay ' + totalOwed.toLocaleString() + ' RWF in ' + days + ' days', () => {
            user.balance += amt;
            user.activeLoan = { principal: amt, interest, totalOwed, dueDate, takenAt: Date.now() };
            DB.saveUser(user);
    ...
    repay() {
        const user = Auth.currentUser();
        if (!user.activeLoan) { UI.toast('No active loan.', 'error'); return; }
        const loan = user.activeLoan;
...
/* ── HISTORY ── */
const History = {
    render() { const u = Auth.currentUser();
        Dashboard.renderTxList('full-tx-list', DB.getUserTxns(u.id), u.id); }
};
...
const Modal = {
    confirm(msg, cb) {
        ...
        fresh.addEventListener('click', () => { this.close();
            cb(); });
    },
...
function toggleBalance() {
    ...
    const balance = document.getElementById("dash-balance");
    const eye = document.getElementById("balance-eye");

    if (!realBalance) {
        realBalance = balance.innerHTML;
    }

    if (balanceVisible) {
        balance.innerHTML = "•••••• <span>RWF</span>";
