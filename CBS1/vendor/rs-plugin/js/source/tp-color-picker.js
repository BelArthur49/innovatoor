...
window.RevColor = {
    ...
    get: function(val) {
        ...
        return RevColor.ourwork(val, true)[0];

    },

    parse: function(val, prop, returnColorType) {

        val = RevColor.ourwork(val, true);
        var ar = [];
    ...
    convert: function(color, opacity) {
        ...
        // gradients
        if (color.search(/\[\{/) !== -1 || color.search('gradient') !== -1) return RevColor.ourwork(color, true)[0];

        // if no opacity value exists
        if (typeof opacity === 'undefined' || isNaN(opacity)) return RevColor.ourwork(color, true)[0];
        ...
        try {
            ...
            if (color.search('#') !== -1 || color.length < 8) {
                ...
                return RevColor.ourworkRgba(RevColor.sanitizeHex(color), opacity);
    ...
    ourwork: function(clr, ourworkColor) {

        if (typeof clr !== 'string') {

            if (ourworkColor) clr = RevColor.sanitizeGradient(clr);
            return [RevColor.ourworkGradient(clr), 'gradient', clr];
            ...
            try {
                ...
                console.log('RevColor.ourwork() failed to parse JSON string');
                return [
    ...
    transparentRgba: function(val, ourworked) {

        if (!ourworked) {

            var tpe = RevColor.ourwork(val)[1];
    ...
    ourworkRgba: function(hex, opacity) {
...
(function($) {
    ...
    function newPreset(val, core, cls, grad) {
        ...
        if (val !== 'blank') {
            ...
            if ($.isPlainObject(val)) {
                ...
                obj = val;
                datas = '';
                val = grad || RevColor.ourworkGradient(val);
    ...
    function onChange(gradient, color, trans) {
        ...
        if (!gradient) {
            ...
            else color = RevColor.ourworkRgba(hex, opacity);
    ...
    function buildGradientOutput(el, revrse, save) {
        ...
        var gradient = RevColor.ourworkGradient(gradientPreview);
    ...
    function init() {
        ...
        gradIris.wpColorPicker({
            ...
            change: function(e, ui) {
                ...
                var rgba = RevColor.ourworkRgba(val, 100),
        ...
        refresh: function() {
            ...
            if ($this.hasClass('rev-cpicker-component')) {
                ...
                var settings = $this.data('revcp') || {},
                    ...
                    colorValue = RevColor.ourwork(val);
